...
require.scopes["icon"] = (function () {
    ...
    function renderFrames(notificationType) {
        ...
        return Promise.all([loadImage("icons/abp-16.png"), loadImage("icons/abp-19-whitelisted.png"), loadImage("icons/abp-19-notification-" + notificationType + ".png"), loadImage("icons/abp-38.png"), loadImage("icons/abp-38-whitelisted.png"), loadImage("icons/abp-38-notification-" + notificationType + ".png")]).then(function (images) {
            var images = {
                19: {
...
require.scopes["prefs"] = (function () {
    ...
    defaults.subscriptions_fallbackurl = "http://ad-blocker.org/getSubscription?version=%VERSION%&url=%SUBSCRIPTION%&downloadURL=%URL%&error=%ERROR%&channelStatus=%CHANNELSTATUS%&responseStatus=%RESPONSESTATUS%";
    defaults.subscriptions_autoupdate = true;
    defaults.subscriptions_exceptionsurl = "http://cdn.ad-blocker.org/adb/exceptionrules.txt";
    defaults.subscriptions_antiadblockurl = "http://cdn.ad-blocker.org/adb/antiadblockfilters.txt";
    defaults.documentation_link = "http://ad-blocker.org/redirect?link=%LINK%&lang=%LANG%";
    defaults.notificationdata = {};
    defaults.notificationurl = "http://cdn.ad-blocker.org/adb/notification.json";
...
require.scopes["whitelisting"] = (function () {
    ...
    var checkWhitelisted = exports.checkWhitelisted = function (page, frame, typeMask) {
        ...
        //var maincurrenturl = getDomain(page.url.host);
        var maincurrenturl = getDecodedHostname(page.url).replace(/^www\./, "");
        if (ExclusionList.indexOf(maincurrenturl) >= 0) {
            var matchedFilter = "@@||" + maincurrenturl + "^$document";
            return matchedFilter;
        }

        var maincurrenturlArray = maincurrenturl.split(".");
        var _DomainNamee = maincurrenturlArray[maincurrenturlArray.length - 2] + "." + maincurrenturlArray[maincurrenturlArray.length - 1];

        if (ExclusionList.indexOf(_DomainNamee) > -1) {
            var matchedFilter = "@@||" + maincurrenturl + "^$document";
            return matchedFilter;
        }
    ...
    Downloader.prototype = {
        ...
        _download: function (downloadable, redirects) {
            ...
            var errorCallback = function errorCallback(error) {
                ...
                Cu.reportError("Ad-block: Downloading URL " + downloadable.url + " failed (" + error + ")\n" + "Download address: " + downloadUrl + "\n" + "Channel status: " + channelStatus + "\n" + "Server response: " + responseStatus);
    ...
    var ElemHide = exports.ElemHide = {
            ...
            if (filter instanceof ElemHideException) {
                ...
                if (usingGetSelectorsForDomain) {
                    ...
                    if (unconditionalFilters) {
                        for (var _loopIndex37 = 0; _loopIndex37 < unconditionalFilters.length; ++_loopIndex37) {
                            var f = unconditionalFilters[_loopIndex37];
        ...
        _generateCSSContent: function () {
            ...
            for (var domain in domains) {
                ...
                if (domain) {
                    _generatorResult38.push(("@-moz-document domain(\"" + domain.split(",").join("\"),domain(\"") + "\"){").replace(/[^\x01-\x7F]/g, escapeChar));
                }
                else {
                    _generatorResult38.push("@-moz-document url-prefix(\"http://\"),url-prefix(\"https://\")," + "url-prefix(\"mailbox://\"),url-prefix(\"imap://\")," + "url-prefix(\"news://\"),url-prefix(\"snews://\"){");
                }
                for (var selector in list) {
                    _generatorResult38.push(selector.replace(/[^\x01-\x7F]/g, escapeChar) + "{" + cssTemplate.replace("%ID%", list[selector]) + "}");
                }
                _generatorResult38.push("}");
            }
            return _generatorResult38;
        },
        unapply: function () {
            if (ElemHide.applied) {
                try {
...
require.scopes["filterStorage"] = (function () {
    ...
    var FilterStorage = exports.FilterStorage = {
        ...
        get sourceFile() {
            ...
            if (!file) {
                try {
                    ...
                    file = IO.resolveFilePath(Services.prefs.getDefaultBranch("extensions.adblocker.").getCharPref("data_directory"));
                    if (file) {
                        file.append("patterns.ini");
                    }
                }
            ...
            if (!file) {
                //Cu.reportError("Ad-blocker: Failed to resolve filter file location from extensions.adblockplus.patternsfile preference");
                Cu.reportError("Ad-blocker: Failed to resolve filter file location from extensions.adblocker.patternsfile preference");
            }
        ...
        _generateFilterData: function (subscriptions) {
            var _generatorResult38 = [];
            _generatorResult38.push("# Adblocker preferences");
            _generatorResult38.push("version=" + formatVersion);
...
require.scopes["notification"] = (function () {
    ...
    var Notification = exports.Notification = {
        ...
        _getDownloadables: function () {
            ...
            _generatorResult38.push(downloadable);
            return _generatorResult38;
        },
        ...
        _getDownloadables: function () {
            ...
            for (var _loopIndex74 = 0; _loopIndex74 < FilterStorage.subscriptions.length; ++_loopIndex74) {
                ...
                if (subscription instanceof DownloadableSubscription) {
                    _generatorResult38.push(this._getDownloadable(subscription, false));
                }
            }
            return _generatorResult38;
        },
